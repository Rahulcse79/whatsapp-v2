...
|---|---|---|---|---|
| 2026-09-16 16:09 | `dialplan/default/01_coralx_push_wake.xml`, `dialplan/default/02_cisco_features.xml` | media bypass made conditional: a call whose offer carries `m=video` no longer sets `bypass_media=true`. The `3000` extension is unchanged apart from a comment recording what does **not** work there | a bypassed leg negotiates Lyra, which this build has no codec for; merging REFERs that leg into room `3000`, and FreeSWITCH — having to take media back for the mixer — offered `m=audio 0 RTP/AVP 19` and video alone, so every transferred member joined the canvas deaf and mute on a placeholder `L16/0` codec pointed at `127.0.0.1:9999`. Audio-only calls still bypass and still get Lyra ([conference-video/](conference-video/README.md)) | `…/01_coralx_push_wake.xml.bak.20260916-160938`, `…/02_cisco_features.xml.bak.20260916-160402` |
| 2026-09-15 13:22 | `autoload_configs/conference.conf.xml`, `autoload_configs/conference_layouts.conf.xml`, `dialplan/default/02_cisco_features.xml` | new profile `whatsapp-video` (`video-mode=mux`, 720x1280 portrait canvas, 15 fps, one shared encode), new layout group `wa-portrait` (`wa-1`…`wa-9`, all `zoom="true"`), and `3000` pointed at the new profile | room `3000` ran in the `default` profile, which sets no `video-mode`; mod_conference's default is passthrough, so it forwarded only the floor holder's camera and three handsets all saw the same one face ([conference-video/](conference-video/README.md)) | `…/conference.conf.xml.bak.20260915-132219`, `…/conference_layouts.conf.xml.bak.20260915-132219`, `…/02_cisco_features.xml.bak.20260915-132219` |
| 2026-09-13 15:35 | `dialplan/default/01_coralx_push_wake.xml`, `dialplan/coralx.xml` | `bypass_media=true` set before the bridge in the push-wake rule and in `coralx-resume` | every handset-to-handset call now passes its SDP through untouched, so any pair — not only 1001↔1002 — can negotiate Lyra; verified with scripted Lyra-only UAs on both the direct and the park → resume path ([04](04-dialplan.md)) | `…/01_coralx_push_wake.xml.bak.20260913-153513`, `dialplan/coralx.xml.bak.20260913-153513` |
| 2026-09-13 15:35 | `dialplan/default.xml` | `cfwd_master`, `agent_1003`, `agent_1004` removed from the context body | they matched 1003/1004 ahead of the include, their `hash` calls were wrong in both forms (always `-ERR`), and their bridge had no media bypass — a Lyra-only 1002→1003 got 488 ([04](04-dialplan.md)) | `dialplan/default.xml.bak.20260913-153513` |
| 2026-09-13 15:35 | `dialplan/default/00_cfwd.xml`, `dialplan/default/00_whatsapp_v2_bypass.xml` | **retired** (renamed to `.bak.20260913-153513`) | the duplicate `cfwd_master`, and the pair-specific bypass rule that also shadowed push wake for 1001↔1002 | rename back to restore |
...
| Canonical copies of the conference-video FreeSWITCH files | `docs/Freeswitch_configuration_docs/conference-video/` | 2026-09-15 |
