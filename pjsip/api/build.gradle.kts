 ...
 *
 * Generated from **pjproject 2.17** by **SWIG 4.2.0**, by the `bindings` job in
 * `.github/workflows/build-pjsip.yml`. Regenerate by running that workflow and taking the
 * `pjsua2-java-api-<version>` artifact wholesale; do not hand-edit anything under `src`,
 * and do not generate it on a workstation.
 *
 * The SWIG version is not incidental. It is whatever `apt-get install swig` gives the
 * runner, and it must be the same one that builds the AAR, because the `.so` exports
 * `Java_org_pjsip_pjsua2_pjsua2JNI_*` symbols named after the Java it was generated
 * beside. Between 4.2.0 and 4.4.1, for one example, the typed vector classes rename
 * `add`/`reserve` to `doAdd`/`doReserve` - 45 of these files differ. An API generated by
 * a different SWIG than the binary is an `UnsatisfiedLinkError` waiting for the first
 * call, so this is taken from the CI job that also runs `make` in `pjsip-apps/src/swig`,
 * and the job prints `swig -version` into its log so a runner-image bump is visible
 * rather than silent.
 *
