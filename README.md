...
**CI decides.** Push to `main` or open a PR and the **CI** workflow builds and checks
everything, on Linux, for all three ABIs. Nothing is "done" until it is green there. The
Gradle wrapper binary is generated by the **Bootstrap Gradle wrapper** workflow (run once,
manually), because a `.jar` cannot be authored by hand.

**But build locally too, because CI cannot see everything.** A local run on macOS found five
...
The rule of thumb: **run the JVM half locally, leave the native half to CI.** Kotlin
compilation, unit tests and detekt give feedback in a minute and catch the things that
otherwise cost an eight-minute CI round trip. The native cross-compile is the opposite —
slow, and green on macOS proves little about Linux.
...
|---|---|---|
| `-Ppjsip.abis=arm64-v8a` | Builds one ABI instead of three | ~20 min instead of ~60. CI passes no override, so the N-6 packaging assertion still sees all three. A one-ABI APK simply will not install elsewhere — loud, not silent |
...
| **Android NDK** | **r27c** (`27.2.12479018`) | `.github/workflows/ci.yml`, and **asserted** at build time |
